...
namespace http_handler {
    ...
    void RequestHandler::SetResponceStatus(const std::string requestTarget, http::status& status) {
        ...
        if (IsApiRequest(requestTarget)) {
            ...
            if (IsFileExist(GetFilePath(requestTarget))) {
                status = http::status::ok;
            }
            else {
                status = http::status::not_found;
    ...
    StringResponse RequestHandler::PrepareStringResponce(const std::string requestTarget, unsigned http_version, bool isKeepAlive) {
