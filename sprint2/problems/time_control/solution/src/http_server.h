...
// Разместите здесь реализацию http-сервера, взяв её из задания по разработке асинхронного сервера
    ...
    protected:
        ...
        void Write(http::response<Body, Fields>&& response) {
            ...
            std::string type { response.base()[http::field::content_type] };
            ...
            std::chrono::system_clock::time_point end_ts_ = std::chrono::system_clock::now();
            auto response_time = std::chrono::duration_cast<std::chrono::milliseconds>(end_ts_ - start_ts_);
            long long res_t = response_time.count();        

            Logger::LogResponse(ip, res_t, code, type);
        };

        void SetStartResponseTime(std::chrono::system_clock::time_point& time) {
            start_ts_ = time;
        };

        std::string GetSessionRemoteIP() {
    ...
    private:
        ...
        void HandleRequest(HttpRequest&& request) override {
            ...
            std::string ip = GetSessionRemoteIP();
            std::string method{ request.method_string() };
            std::string req{ request.target() };
            Logger::LogRequest(ip, req, method);
            std::chrono::system_clock::time_point start_time = std::chrono::system_clock::now();
            SetStartResponseTime(start_time);
